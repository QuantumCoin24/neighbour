...
import { NeighbourCard, NeighbourBadge, NeighbourButton } from '@neighbour/design-system';
...
export default function EventPreview({ communityId }: Props) {
  ...
  return (
    ...
    >
      ...
      {events.length === 0 ? (
        ...
        events.map((event) => (
          ...
          >
            ...
            <NeighbourBadge>📅 {new Date(event.startsAt).toLocaleDateString()}</NeighbourBadge>
          </div>
        ))
      )}
      ...
      >
        <NeighbourButton>View Events</NeighbourButton>
      </div>
    </NeighbourCard>
  );
}
