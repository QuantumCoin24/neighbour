...
export class InMemoryMembershipRepository implements MembershipRepository {
  ...
  async save(membership: MembershipEntity): Promise<MembershipEntity> {
    const existing = this.memberships.find(
      (item) =>
        item.userId === membership.userId && item.neighbourhoodId === membership.neighbourhoodId,
    );
  ...
  async remove(userId: string, neighbourhoodId: string): Promise<void> {
    this.memberships = this.memberships.filter(
      (item) => !(item.userId === userId && item.neighbourhoodId === neighbourhoodId),
    );
  }
  ...
  async findMembers(neighbourhoodId: string): Promise<MembershipEntity[]> {
    return this.memberships.filter((item) => item.neighbourhoodId === neighbourhoodId);
  }
}
