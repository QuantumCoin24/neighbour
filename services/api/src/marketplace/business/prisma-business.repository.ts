...
export class PrismaBusinessRepository extends BusinessRepository {
  ...
  private map(business: any): BusinessEntity {
    return {
      id: business.id,
  ...
  async save(business: BusinessEntity): Promise<BusinessEntity> {
    const record = await this.database.business.create({
      data: {
        id: business.id,
  ...
  async findById(id: string): Promise<BusinessEntity | undefined> {
    const record = await this.database.business.findUnique({
      where: {
        id,
      },
    });
  ...
  async findByOwner(ownerId: string): Promise<BusinessEntity | undefined> {
    const record = await this.database.business.findFirst({
      where: {
        ownerId,
      },
    });
  ...
  async search(query: string): Promise<BusinessEntity[]> {
    const records = await this.database.business.findMany({
      where: {
        OR: [
          {
