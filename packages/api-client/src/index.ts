export const API_BASE_URL = process.env.NEIGHBOUR_API_URL ?? 'http://localhost:4000/api/v1';
...
export async function apiRequest<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    console.error('Neighbour API Failure:', response.status, body);

    throw new Error(`Neighbour API Error ${response.status}`);
  }
